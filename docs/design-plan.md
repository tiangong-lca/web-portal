...
- 共享 SEO checker 以 `--loopback-preview` 模式在既有 preview fixture 服务上运行：不新增构建或 runtime，不切换 production fixture，也不改变现有 noindex/Dataset 语义；该模式只接受 loopback origin、要求显式 sitemap 路径，并断言 Disallow 与逐页 noindex。
