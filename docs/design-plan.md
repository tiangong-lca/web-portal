...
| 有界读协调器（`src/server/data/read-coordinator.ts`） | 昂贵 Search/Facets/navigation 读的同参并发合并、实例内准入、失败冷却，以及"超过 30 秒窗口即等待受控回源或返回暂不可用"的可见性铁律；经 `unstable_cache`（`src/server/data/cache-boundary.ts`）复用同一 Next Incremental Cache，stale 后台刷新同样走应用 loader | 不是跨实例全局限流（多实例上限按实例数放大，全局成本由 Database 侧查询有界性负责）；不缓存失败；不把 key/原 query/UUID 写入日志；详情/版本/LCIA/sitemap 与首页 ISR 调用点仍走原 fetch 缓存路径（首页因 `no-store` 回源会破坏静态渲染而显式保留 legacy 边界） |
...
7. 日志不含原始自然语言、embedding、完整 UUID 列表或 secret；HMAC/guard/budget/concurrency/fallback 指标和告警可区分；Portal catalog 有界读另外区分 consumer/origin 事件、真实 `cacheHit`（`origin|coalesced|cache` 口径）、只含封闭枚举的 `queryShape/hasFilters/hasCursor/sort` 形状标签，以及实例本地 `local_capacity_shed`/`local_cooldown_shed`（不复用 Edge 全局 `concurrency_exhausted`）；
