...
Run `pnpm check:maps` after changing `scripts/maps/**`, the geography dictionary or generated map assets. It verifies offline regeneration, source receipts, content hashes and the per-layer 150 KiB gzip budget. `mapshaper` is a pinned build-only dependency; optional SQLite/native acceleration builds are disabled because this pipeline only reads GeoJSON and emits SVG paths. Use `node scripts/maps/build-region-maps.mjs --fetch` only for an intentional reviewed upstream refresh. Runtime assets must stay layer-separated and the browser must never fetch raw sources.
