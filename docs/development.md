...
### Accepted MapLibre geography reference

The accepted Portal #111 prototype remains in `.storybook/maplibre-reference/` and `.storybook/stories/maplibre-explorer.stories.tsx` for design reference. Portal #113 uses the same exact MapLibre version as a runtime dependency through the separately owned production renderer described below; the prototype component and count snapshot never enter a production route. The Vite worker uses MapLibre's separate worker entry through `?worker&url`. There is no MapTiler key, external tile request, terrain or lighting dependency. The reviewed BSD/MIT/Apache notices for MapLibre and its `@maplibre/mlt` dependency are retained in `NOTICE.txt` and copied into the Storybook output; the license-policy exception is scoped to that exact package name.
...
`pnpm check:maps` verifies both the retained reviewed SVG mapping inputs and the production MapLibre assets. `scripts/maps/build-maplibre-maps.mjs` regenerates 29 EPSG:4326 GeoJSON layers, a standalone same-origin MapLibre ES worker and license notices in `public/maps/gl/`. All outputs are content-hashed and preserve reviewed geometry identities, source receipts and China interaction aliases. GeoJSON content uses `.json` asset URLs: EdgeOne serves `.geojson` as uncompressed octet-stream, while standard JSON MIME participates in HTTP compression. Hosted qualification checks actual encoded bytes as well as the offline gzip budget. `--check` rebuilds offline, compares canonical raw hashes and enforces each geometry's 150 KiB gzip ceiling. Gzip measurements are reported rather than embedded in the canonical manifest because zlib platforms can differ slightly. The output worker uses a `.js` extension because EdgeOne serves `.mjs` as `application/octet-stream`; the worker stays an ES module and its hash covers the unchanged bytes. Hosted qualification must verify its JavaScript Content-Type as well as its hash. The worker build uses the installed Vite API with `configFile:false`, `envDir:false`, `publicDir:false`, no sourcemaps and one self-contained output; it must introduce no remote imports or eval requirement. Generated assets have a precise Prettier exclusion.

The production scene loads through `next/dynamic` in `RegionExplorer`; MapLibre is a pinned runtime dependency, while Mapshaper and Vite remain build-only tools. The browser never downloads raw sources or the complete vocabulary, and home/ordinary search must not fetch map geometry or workers. MapLibre CSS is loaded with the scene, so the canvas host has a more specific position/size rule than the SDK's map container default. Real rendered dimensions, worker CSP/MIME behavior, viewport framing and failure-list fallback require browser checks. Story afterEach hooks wait for actual map idle before review thumbnails freeze.

`POST /internal/navigation` is a bounded, same-origin, anonymous UI adapter for a classification branch. It accepts at most 8 KiB, validates the existing V3 filters, reads at most 50 children through `portal_navigation_v1`, and returns at most 64 KiB of current-locale labels/counts/cursor. It never echoes complete href/query strings per node or exposes raw data. Its POST response is no-store; the underlying public RPC cache remains 30 seconds. The client also deduplicates and caches branch pages for 30 seconds, scopes them by full context and cancels/ignores obsolete requests. Initial RSC rendering reads only the root and current ancestor path; a selected child outside a first page remains visible using its own counted parent receipt. Native links provide no-JS navigation and cursor fallback.

Navigation changes require the Database snapshot check, classification contract/cache tests, production journeys and Storybook disclosure/map scenarios, with V2/Hybrid regression and four-locale parity preserved. The 30 KiB map-interaction limit was explicitly waived for MapLibre adoption; homepage/search initial JS budgets remain enforced through lazy loading. `scripts/maps/sources` remains the source boundary; refresh upstream data only through the separate reviewed fetch workflow.
...
Production geography browser tests use real MapLibre frames in ordinary CI. Both the production and Storybook runners use `channel: "chromium"` for the full Chromium headless renderer; Playwright selects ANGLE/SwiftShader in CI, or locally with `PORTAL_E2E_SOFTWARE_WEBGL=1`. `PORTAL_WEBGL_TESTS=1` opts into the homepage performance measurement independently of the renderer, so local GPU measurements need not be forced through software rendering. Production CI runs one browser worker so software WebGL and paint samples do not compete with another page. The performance probe waits for an actual LCP receipt before the first input, which otherwise ends LCP collection before any paint is reported. Metric thresholds are unchanged. This does not opt ordinary CI into the separately gated homepage cinematic performance lane.

The MapLibre world overview explicitly uses Mapshaper’s official `planar` simplification metric: the default spherical metric produced different retained vertices across macOS and Linux despite byte-identical source data. The 28 China/province layers retain their reviewed generation calls and bytes. The manifest records simplification percentages, planar layers, coordinate system and output precision option; `--check` still verifies every canonical byte, source identity and layer budget.
