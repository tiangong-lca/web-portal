...
Use the normal Storybook AI test/review workflow for the globe, original SVG, world-to-city journey, small regions, dark theme, mobile and unavailable-WebGL scenes. The local prototype lifts the former 30 KiB map-interaction limit only for this experiment; production bundle budgets and the existing production renderer remain unchanged. MapTiler commercial terms and quota sizing need a separate decision if hosted tiles are proposed later.
