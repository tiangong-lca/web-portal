...
`Design references/MapLibre geography` 是独立审阅样板，代码、计数快照和边界生成器位于 `.storybook/maplibre-reference/`，不接入生产目录。MapLibre 与原有 SVG 使用相同已审阅的地区身份、边界来源和过程公开版本计数；四语文字使用普通 HTML，保留真实零值以及无精确边界范围的列表入口。快照包含历史公开版本，显示采集时间，不冒充实时接口。

世界层默认静止朝向东亚及西太平洋，可切换地球、平面、原地图和地区列表。点选预览数量并推进相机，明确的“浏览下级地区”操作进入中国或省级城市边界；下钻和返回复用同一个 MapLibre 实例，缩放时从球面逐渐过渡到平面。相邻国家和省份仍以浅色底图提供上下文。数量填色为全不透明平面色，不加入地形、昼夜或光照。世界层沿用中国大陆、台湾、香港、澳门及南海补充几何的同一导航身份；中国层保留各自真实地区身份，并为台湾、香港、澳门提供 44px 以上的可聚焦快捷目标。

窄屏默认列表，地图使用协作手势以保留页面滚动。减少动态效果时跳到目标视野，WebGL 不可用时保留完整列表及数据入口。无障碍检查覆盖明暗主题、键盘、小目标和失败场景。原地图切换用于视觉对照；通过样板评审不代表获准替换生产地图或接入收费底图服务。
...
同源 Next 链接通过共享反馈组件显示局部等待标记，搜索、排序、筛选抽屉和核对导航使用 React transition 保留现有内容。约 150ms 后仍未完成才显示细进度条及四语礼貌播报；不遮挡页面、不使用虚假百分比。取消、完成、组件卸载均清除相应状态；减少动态效果时保留静态提示。地图选择本身是即时局部行为，明确下钻才启动导航反馈。中国及省市图在画布上方和两侧保留 24–40px 响应式留白，底部空间容纳紧凑图注；世界图保留太平洋居中的完整轮廓。
