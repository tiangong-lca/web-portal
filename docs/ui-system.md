...
`Design references/MapLibre geography` 是独立审阅样板，代码、计数快照和边界生成器位于 `.storybook/maplibre-reference/`，不接入生产目录。MapLibre 沿用已审阅的地区身份、边界来源和过程公开版本计数；四语文字使用普通 HTML，保留真实零值以及无精确边界范围的列表入口。快照包含历史公开版本，显示采集时间，不冒充实时接口。

世界层默认静止朝向东亚及西太平洋，可切换地球、平面和地区列表。点选预览数量并推进相机，明确的“浏览下级地区”操作进入中国或省级城市边界；下钻和返回复用同一个 MapLibre 实例，缩放时从球面逐渐过渡到平面。相邻国家和省份仍以浅色底图提供上下文。数量填色为全不透明平面色，不加入地形、昼夜或光照。世界层沿用中国大陆、台湾、香港、澳门及南海补充几何的同一导航身份；中国层保留各自真实地区身份，并为台湾、香港、澳门提供 44px 以上的可聚焦快捷目标。

窄屏默认列表，地图使用协作手势以保留页面滚动。减少动态效果时跳到目标视野，WebGL 不可用时保留完整列表及数据入口。无障碍检查覆盖明暗主题、键盘、小目标和失败场景。地球与平面使用相同边界和数据进行对照；Portal #113 经用户确认后将该方案用于生产，生产组件、实时计数和加载规则由下文“层级分类与地区地图”约束；样板快照保持隔离，未接入收费底图服务。
...
过程和流在现有目录布局中复用 `ClassificationNavigation`，基于官方 `@shadcn` radix-nova `Collapsible` 组合语义 nav/ul/li。展开按钮和原生分类链接并列；箭头只展开，链接才改变右侧结果，父分类同样可进入。展开状态按 process/flow 分开保存，并由搜索区外的 `ClassificationNavigationProvider` 持有，避免模式区重建时丢失。当前选择仍由 URL 表达，刷新恢复祖先路径；不声明完整 TreeView，也不添加 role=tree。

初始 HTML 只包含根及当前路径所需的分支，客户端展开时按父节点读取一层，缓存收起的分支，并在分支内追加续页；超过 50/100 个节点不得被静默截断。缓存键包含类型、查询、筛选和语言，旧请求会取消或忽略；错误显示在分支内并提供重试。响应仅含当前语言标签和计数，不传整份词表或重复查询链接。无 JavaScript 时分类名称和续页仍为真实链接。数量来自公开版本聚合，重复分类路径和父级直属记录口径保持不变。

地区地图是原生列表的渐进增强。MapLibre 仅在显示地图时按需加载，使用同源、散列命名的 worker 和按层 GeoJSON；不接第三方瓦片、地图账号或远程投影服务。世界提供静止朝向东亚/西太平洋的地球及太平洋居中的平面视图；平面视野按画布宽度取景，避免宽屏同时重复显示多个世界。国家、省市沿同一个相机推进，随着放大逐渐转为平面；初始定位与减少动态效果直接到位。数据颜色使用不透明语义色，不受光照或地形影响。

世界层的大陆、台湾、香港、澳门及已有南海图形共用中国的选择身份和计数。进入中国层后各地区使用自己的节点与精确数量，零数量仍可选择。台湾、香港、澳门另有普通 HTML 快捷目标；名称、数量和动作始终保持水平文字。未知、历史及跨区范围继续从列表进入，不强配多边形或伪造零值。手机默认列表，启用地图后使用协作手势保留页面滚动。

地图几何与计数更新分开：筛选或主题变化不重建相机、不重新解析边界；切换层级时旧几何仅作中性背景，停用旧计数及交互。明确选择才进行礼貌播报及必要的操作区可见性调整，悬停不重复播报。地图程序包的两段延迟加载均保留取景区高度并显示本地化加载文字；地图、worker 或数据加载失败仍保留地区列表和明确说明。
...
同源 Next 链接保留共享 pending 状态，但只显示页面顶部统一进度条，不绘制按钮、搜索提交或 Logo 下方的局部短线；搜索、排序、筛选抽屉和核对导航使用 React transition 保留现有内容。约 150ms 后仍未完成才显示细进度条及四语礼貌播报；不遮挡页面、不使用虚假百分比。取消、完成、组件卸载均清除相应状态；减少动态效果时保留静态提示。地图选择本身是即时局部行为，明确下钻才启动导航反馈。国家和省市的相机取景保留响应式边距，地图下沿提供紧凑信息栏。

地图信息栏用约 12px 次要文字：匹配公开版本数、渐变、少到多与说明按钮靠左，Natural Earth / DataV GeoAtlas 来源靠右，共用基线和地图内边距。窄屏把整个来源组移至下一行；长说明不与链接竞争同行空间。完整说明由键盘和触控均可展开的 Collapsible 呈现，解释零数量、未知/未加载数量、历史公开版本及跨区/历史范围。轮廓统一为实线，缺失数据不改变边界样式。
