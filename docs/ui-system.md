...
地区地图是原生列表的渐进增强。MapLibre 仅在显示地图时按需加载，使用同源、散列命名的 worker 和按层 GeoJSON；不接第三方瓦片、地图账号或远程投影服务。世界提供静止朝向东亚/西太平洋的地球及太平洋居中的平面视图。国家、省市沿同一个相机推进，随着放大逐渐转为平面；初始定位与减少动态效果直接到位。数据颜色使用不透明语义色，不受光照或地形影响。
